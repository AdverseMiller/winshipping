...
The bundled launcher is resolved relative to `build/winshipping`, so a clone can be placed anywhere. It requires `zsh`, `jq`, `virsh`, and a Turing-family NVIDIA display function bound to VFIO. Privilege escalation tries `/bin/csu`, `pkexec`, then `sudo`. `WINSHIPPING_BOX_RENDERER` can select another launcher; `BAR1_SCANNER`, `BAR1_WDDM_PROBE`, `BAR1_VIRSH`, `BAR1_DOMAIN`, and `BAR1_QEMU_PID_FILE` override individual host components. The optional `/dev/tu104-bar1-wc` helper is not required because the renderer retains its VFIO BAR1 mapping fallback.
