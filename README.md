...
C++ UE5 actor-position monitor and targeting prototype for the `win11` Windows VM. It loads the installed `memflow-qemu` and `memflow-win32` plugins through memflow's official C++ ABI.
...
The first build fetches the official memflow `0.2.4` source, builds its C++ FFI archive, and builds the bundled TU104 BAR1 renderer. The renderer protocol header, renderer source, hardware scanner, and patched WDDM ownership probe are all stored under `tools/`; the build and launcher contain no user-specific source-tree paths. The application itself is C++17; Cargo is only used to build the upstream memflow ABI library. At runtime, memflow discovers plugins through its standard per-user installation directory, `MEMFLOW_PLUGIN_PATH`, and the current working directory. For a nonstandard installation, provide one or more plugin directories through `MEMFLOW_PLUGIN_PATH`:
